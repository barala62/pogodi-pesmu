...
export function saveGameResult(isWon: boolean, guessStep: number, dateStr: string): UserStats {
  ...
  if (isWon) {
    next.gamesWon += 1;
    // Streak logic: check if played consecutive days
    const isConsecutive = isYesterday(current.lastPlayedDate, dateStr);
    if (isConsecutive || current.lastPlayedDate === dateStr) {
